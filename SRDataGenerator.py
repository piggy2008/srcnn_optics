...
class SRDirectoryIterator(Iterator):
    '''
    ...
    def __init__(self, file_path, seg_data_generator,
        ...
        super(SRDirectoryIterator, self).__init__(
    ...
    def next(self):
        ...
        for i, j in enumerate(index_array):
            ...
            if self.target_size:
                if self.crop_mode != 'none':
                    ...
                    if self.label_file_format is not 'npy':
                        y = img_to_array(label, data_format=self.data_format)
                    ...
                    if self.label_file_format is not 'npy':
                        y = img_to_array(label.resize((self.target_size[1], self.target_size[0]), Image.NEAREST), data_format=self.data_format)
                    else:
            ...
            x, y = self.seg_data_generator.normalize(x, y)
